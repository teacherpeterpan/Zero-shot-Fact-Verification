...
class QGPipeline:
    ...
    # generate a batch of examples at one time
    # Input format: 
    #   inputs: ['src1', 'src2', ...]
    #   answers: ['ans1', 'ans2', ...]
    def batch_qg_with_answer(self, inputs: list, answers: list):
        # preprocessing
        qg_examples = []
        
        for ind, source in enumerate(inputs):
            source = " ".join(source.split())
            sents = sent_tokenize(source)
            answer_text = answers[ind]
...
if __name__ == "__main__":
    test_passage = '''PERSON (born 19 January 1980) is a British racing driver and former Formula One driver. 
    ...
    nlp = pipeline("question-generation", model='valhalla/t5-base-qg-hl', qg_format="highlight", gpu_index=0)
